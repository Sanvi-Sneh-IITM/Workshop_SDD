...
def search_projects(
    q: Optional[str] = Query(None, description="Search project title and description"),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    ...
    if q and q.strip():
        term = f"%{q.strip()}%"
        query = query.filter(
            or_(Project.title.ilike(term), Project.description.ilike(term))
        )
    return query.all()
...
def search_tasks(
    q: Optional[str] = Query(None, description="Search task title and description"),
    project_id: Optional[int] = Query(None, description="Filter by project ID"),
    status: Optional[TaskStatus] = Query(None, description="Filter by task status"),
    due_after: Optional[datetime] = Query(None, description="Tasks due on or after this datetime"),
    due_before: Optional[datetime] = Query(None, description="Tasks due on or before this datetime"),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    query = db.query(Task).join(Project).filter(Project.owner_id == current_user.id)
